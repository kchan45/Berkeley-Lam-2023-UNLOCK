...
class Simulation():
    ...
    def run_closed_loop(self, c,
        ...
        #
        Yhat = np.zeros_like(Ysim) #define
        #
        ...
        Ysim[:,0] = np.ravel(self.hp(Xsim[:,0],Vsim[:,0],0, 100).full())
        ...
        for k in range(self.Nsim):
            ...
            if mpc_controller:
                if dose:
                    if multistage:
                        ...
                        #prob_info['Np'] = prob_info['Np'] - k
                        #c = EconomicMPC(self.prob_info)
                        #c.get_mpc()
                        
                        #Modification done
                        c.set_parameters([Yhat[:,k], Yrefsim[:,k], dose_sim[:,k]])
                        #Modification done
                else:
                    if offset:
            ...
            if k<100:
                Ysim[:,k+1] = np.ravel(self.hp(Xsim[:,k+1],Vsim[:,k+1], 0, 100).full())
            else:
                Ysim[:,k+1] = np.ravel(self.hp(Xsim[:,k+1],Vsim[:,k+1], (k-100), 100).full())
